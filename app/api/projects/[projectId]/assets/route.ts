...
import { inferAssetType, safeFilename, safeRemoteUrl, validateAssetType } from "@/lib/server/assets";
import { apiContext, type RouteContext } from "@/lib/server/context";
import { mediaBucket } from "@/lib/server/runtime";
import { allRows, requireOwnedProject, serializeAssetById, touchProject } from "@/lib/server/store";
...
async function createMetadataAsset(request: Request, db: D1Database, projectId: string): Promise<Response> {
  ...
  const type = validateAssetType(body.type);
  ...
  await db.prepare(`INSERT INTO assets (id, project_id, name, type, description, mime_type, size_bytes, storage_key, source_url, thumbnail_url, metadata_json, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NULL, NULL, NULL, ?, ?, ?, ?, ?, ?)`).bind(assetId, projectId, name, type, description, sourceUrl ?? null, thumbnailUrl ?? null, jsonText(metadata, {}), status, now, now).run();
...
async function uploadAsset(request: Request, db: D1Database, projectId: string): Promise<Response> {
  ...
  const type = requestedType ? validateAssetType(requestedType) : inferAssetType(file.type);
  ...
  try {
    await db.prepare(`INSERT INTO assets (id, project_id, name, type, description, mime_type, size_bytes, storage_key, source_url, thumbnail_url, metadata_json, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, 'ready', ?, ?)`).bind(assetId, projectId, name, type, description, file.type || "application/octet-stream", file.size, storageKey, jsonText(metadata, {}), now, now).run();
  } catch (error) {
