...
export async function DELETE(request: Request, context: RouteContext<{ projectId: string }>): Promise<Response> {
  try {
    ...
    const stored = await db.prepare(`SELECT storage_key AS storageKey FROM assets WHERE project_id = ? AND storage_key IS NOT NULL`).bind(projectId).all<{ storageKey: string }>();
    ...
    await db.batch([
      db.prepare(`DELETE FROM projects WHERE id = ? AND owner_id = ?`).bind(projectId, identity.userId),
      db.prepare(`UPDATE workspaces SET active_project_id = ?, updated_at = ? WHERE user_id = ?`).bind(next?.id ?? null, nowIso(), identity.userId),
    ]);
    if (bucket) {
      try {
